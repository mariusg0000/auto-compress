 ...
 * WHY:     Ensures summary quality follows the currently active session model.
 * HOW:     Ignores plugin-level model override and derives model from active message history.
 * PARAMS:  configuredModel: object|string|undefined — Unused; kept only for backward-compatible call sites.
...
- If a new unfinished task appears, put it under Current task.
  ...
  return {
    "experimental.chat.messages.transform": async (_input, output) => {
      ...
      if (sessionState.summary) {
        const summaryTextFormatted = `### 📌 SYSTEM NOTICE: CONTEXT COMPACTION (SUMMARY OF PRUNED HISTORY)\n> [!IMPORTANT]\n> The older chat history has been pruned to optimize the context window. Below is a dense technical summary of the work, technical decisions, and status from the pruned turns. Do not reply to this message.\n\n\`\`\`xml\n<pruned_history_summary>\n${sessionState.summary}\n</pruned_history_summary>\n\`\`\``;
...
======================================================================
      ...
      writeDebugLog(debug, sessionID, beforeTokens, beforeTokens, messages.length, afterTokens, removedIndices, messages);
